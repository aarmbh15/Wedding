...
>
  <div className="max-w-3xl mx-auto">
    <div className="text-center mb-16">
      <p className="text-[0.6rem] uppercase tracking-[0.3em] text-[#c9a84c] mb-2">
        Support
      </p>
    ...
    <div className="space-y-4">
      {faqData.map((faq, i) => (
        <div
          key={i}
          className="bg-[#F4F1EA] border border-black/5 overflow-hidden"
        >
          <button
            onClick={() => setOpenFaq(openFaq === i ? null : i)}
            className="w-full px-8 py-6 text-left flex justify-between items-center group"
          >
            <span className="font-['Cormorant_Garamond'] text-lg group-hover:text-[#c9a84c] transition-colors">
              {faq.q}
            </span>
          ...
          >
            <div className="space-y-4">
              {faq.a.map((paragraph, pIdx) => (
                <p
                  key={pIdx}
                  className="text-gray-500 font-light text-sm leading-[1.9]"
                >
                  {paragraph}
                </p>
              ))}
